...
if __name__ == '__main__':
    ...
    for i, batch in enumerate(tqdm(dloader_test()), 1):
        ...
        emb_support_nofilm_np = emb_support.detach().cpu().numpy()
        emb_query_nofilm_np = emb_query.detach().cpu().numpy()
        emb_task_np = emb_task.detach().cpu().numpy()
        ...
        emb_support_np = emb_support.detach().cpu().numpy()
        labels_support_np = labels_support.detach().cpu().numpy()
        emb_query_np = emb_query.detach().cpu().numpy()
        labels_query_np = labels_query.detach().cpu().numpy()
        Kall_np = Kall.detach().cpu().numpy()
        d = dict(
            emb_support_nofilm=emb_support_nofilm_np,
            emb_support=emb_support_np,
            emb_query_nofilm=emb_query_nofilm_np,
            emb_query=emb_query_np,
            emb_task=emb_task_np,
